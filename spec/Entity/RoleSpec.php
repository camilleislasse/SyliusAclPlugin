<?php

/*
 * This file was created by developers working at BitBag
 * Do you need more information about us and what we do? Visit our https://bitbag.io website!
 * We are hiring developers from all over the world. Join us and start your new, exciting adventure and become part of us: https://bitbag.io/career
*/

declare(strict_types=1);

namespace spec\BitBag\SyliusAclPlugin\Entity;

use BitBag\SyliusAclPlugin\Entity\Role;
use BitBag\SyliusAclPlugin\Entity\RoleInterface;
use PhpSpec\ObjectBehavior;

final class RoleSpec extends ObjectBehavior
{
    public function let()
    {
        $this->setCurrentLocale('en_US');
        $this->setFallbackLocale('en_US');
    }

    public function it_is_initializable(): void
    {
        $this->shouldHaveType(Role::class);
    }

    public function it_implements_role_interface(): void
    {
        $this->shouldHaveType(RoleInterface::class);
    }

    public function it_has_empty_array_permissions_by_default(): void
    {
        $this->getPermissions()->shouldReturn([]);
    }

    public function it_adds_permission(): void
    {
        $this->addPermission('create');

        $this->hasPermission('create')->shouldReturn(true);
    }

    public function it_removes_permission(): void
    {
        $this->addPermission('create');
        $this->removePermission('create');

        $this->hasPermission('create')->shouldReturn(false);
    }

    public function it_has_null_name_by_default(): void
    {
        $this->getName()->shouldReturn(null);
    }

    public function it_gets_name(): void
    {
        $this->setName('Accountant');

        $this->getName()->shouldReturn('Accountant');
    }
}
